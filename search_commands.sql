...
# 5 advanced queries
-- 1. Retrieve the names of patients (and the doctors who prescribed it) who have been prescribed medications
-- by doctors specializing in Cardiology
SELECT Patient.Name, Doctor.Name AS DoctorName
FROM Patient
...
WHERE DoctorID IN (SELECT Appointment.DoctorID
                   FROM Appointment
                   JOIN Patient ON Appointment.PatientID = Patient.PatientID
...
SELECT Patient.Name,
       (SELECT SUM(Billing.Amount)
        FROM Billing
        WHERE Billing.PatientID = Patient.PatientID) AS TotalBilled
FROM Patient;
